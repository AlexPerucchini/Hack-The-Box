...
Looks like we have ftp anonymous access with a web-directory file listing that can be accessed via port 80:
...
Originally I created an asp msfvenom file for this server but did not execute, "500 internal server error'. The aspx version produced a reverse shell:
...
#### 3. Open msfconsole and use a multi-handler with the correct payload
```
...
[*] done
```
Selected windows/local/ms10_015_kitrap0d exploit for the root flag!
